...
alias eze='edit $HOME/.oh-my-zsh/custom/enhanced_profile.zsh'   # eze:     edits zsh enhanced profile
...
alias log='mkdir -p $NOTES/log/$(date +"%Y/%m") && touch $NOTES/log/$(date +"%Y/%m/%Y-%m-%d.md")'                                                       # log:          creates a log entry for today ( markdown ) 
alias log-o='mkdir -p $NOTES/log/$(date +"%Y/%m") && touch $NOTES/log/$(date +"%Y/%m/%Y-%m-%d.md") && code -n $NOTES/log'                               # log-o:        creates a log entry for today and opens in editor ( markdown )

alias yesterday='mkdir -p $NOTES/log/$(date -d yesterday +"%Y/%m") && touch $NOTES/log/$(date -d yesterday +"%Y/%m/%Y-%m-%d.md") && code -n $NOTES/log' # yesterday:    creates log entry for yesterday ( same as current log )
